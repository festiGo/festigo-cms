  ...
  def initialize(user)
    ...
    if user.role? :global_admin
      ...
      can [:update, :destroy, :crop], RouteProfile do |profile|
        (user.role? :curator, profile.city) && user.organization == profile.organization
      end
      ...
      can [:update, :destroy, :crop], Location do |location|
        (user.role? :curator, location.network) #&& user.organization == location.organization  #TODO: location has no organization for now
      end
      #can :manage, Reward
