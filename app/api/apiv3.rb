...
module Gohike
  class APIv3 < Grape::API
    version 'v3', using: :header, vendor: 'code_for_europe'
    format :json
    ...
    desc "This allows user to post the check-in saved on their device"
    params do
    ...
    desc "This allows a user to connect his device to the CMS"
    params do
    ...
    desc "This assigns a reward to the user"
    params do
      requires :user_email, type: String, desc: "User email"
      requires :reward, type: Integer, desc: "Reward id"
    end
    post '/assignreward' do
      @user = User.where(:email => params[:user_email]).first_or_initialize
      @reward = Reward.find(params[:reward])
      @user.rewards << @reward
    end

    desc "This lists the rewards for the user"
    params do
      requires :user_email, type: String, desc: "User email"
    end
    post '/myrewards' do
